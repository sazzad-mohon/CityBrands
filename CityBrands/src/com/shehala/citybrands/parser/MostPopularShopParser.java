...
public class MostPopularShopParser {
	...
	public static boolean ParseQuery(Context con, String response)
		...
		AllMostPopularShop.removeAll();
		MostPopularShop mps;

		for (int i = 0; i < holder.length(); i++) {

			final JSONObject mObject = holder.getJSONObject(i);
			mps = new MostPopularShop();
			String result = mObject.getString("shops");
			...
			try {
				...
				PrintLog.getDebugLog("mps.getDistance() : ", mps.getDistance());
			...
			AllMostPopularShop.setPopularShops(mps);
			mps = null;
		}
